...
/**
 * A lightweight presenter for managing state and event-driven behavior using Molecule.
 * Unlike `MoleculeViewModel`, this does not extend `ViewModel` and can be easily composed
 * within other ViewModels by accepting a parent scope.
 *
...
abstract class MoleculePresenter<Event, Model>(scope: CoroutineScope) {
  ...
  /**
   * Helper function for collecting events from a [MoleculePresenter].
   *
   * Collects events emitted by the `events` flow and executes the provided `block` for each event.
   */
  @Composable
  protected fun CollectEvents(block: CoroutineScope.(Event) -> Unit) {
